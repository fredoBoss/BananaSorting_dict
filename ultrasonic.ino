...
// ultrasonic_fixed.ino
// KEY CHANGE: rotateNextSwitchTrig() now sends "CAM_STOP" (not "MOTOR_STOP")
// so Python can distinguish "tray at camera" from "sort complete"
...
void startRotate() { runRotate(true); }
...
void runRotate(boolean en) {
  if (en != motorLastState) {
    digitalWrite(motorCtrlPin, en ? LOW : HIGH);
    motorLastState = en;
  }
}
...
// ─── FIXED: sends "CAM_STOP" not "MOTOR_STOP" ────────────────────────────
// Python's waitForCameraStop() listens for "CAM_STOP" only.
// Python's waitForSortDone()   listens for "SORT_DONE" only.
// These two strings are different → no more race condition.
void rotateNextSwitchTrig() {
  if (testWeight) return;
  ...
  if (rotateOnly) {
    ...
    if (limitSwState1 == LOW) {
      Serial.println("pass rotation");
      Serial.println("CAM_STOP");   // ← was MOTOR_STOP
      return;
    }
  }
  delay(2000);
  ...
  while (isRunning) {
    if (millis() - startTime > timeout) {
      ...
      Serial.println("CAM_STOP");   // ← was MOTOR_STOP (still need to unblock Python)
      isRunning = false;
      break;
    }
    ...
    if (limitSwState1 == LOW) {
      ...
      digitalWrite(motorCtrlPin, HIGH);
      delay(4000);  // camera capture pause
      Serial.println("CAM_STOP");   // ← KEY FIX: was "MOTOR_STOP"
      isRunning = false;
    }
  }
  Serial.println("end while");
